...
void
add_quad_render_commands(Asset_Id texture_asset_id, f32 quad_meter_width, f32 quad_meter_height, Rectangle texture_scissor_rect, V2 world_position, V2 view_vector)
{
	// glInvalidateBufferData()?

	if (!asset_and_dependencies_are_ready(texture_asset_id)) {
		return;
	}

	Loaded_Texture *texture = get_texture(texture_asset_id);
...
{
	auto sprite_data = get_sprite(s.sprite_asset_id);
	...
	add_quad_render_commands(sprite_data->texture_asset_id, f.meter_width, f.meter_height, f.texture_scissor, world_position + f.meter_offset, view_vector);
}
