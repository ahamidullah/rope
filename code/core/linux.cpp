...
void input_key_down(Keyboard *, unsigned, Key_Symbol);
...
platform_update_mouse_position(Mouse *m)
{
	...
	//f32 old_window_x = m->window_x;
	//f32 old_window_y = m->window_y;
	...
	m->window_x = win_x * scaled_meters_per_pixel;
	m->window_y = (window_pixel_height - win_y) * scaled_meters_per_pixel; // Bottom left is zero for us, top left is zero for x11.

	//m->delta_window_x = m->window_x - old_window_x;
	//m->delta_window_y = m->window_y - old_window_y;
