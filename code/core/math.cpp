...
V2
round_to_nearest_pixelx(V2 p)
{
	f32 factor = scaled_meters_per_pixel / 2.0f;

	p.x = round_to_nearest(p.x, factor);
	p.y = round_to_nearest(p.y, factor);

	//p.x = round_to_nearest(p.x, scaled_meters_per_pixel / 2.0f);
	//p.y = round_to_nearest(p.y, scaled_meters_per_pixel / 2.0f);
